...
def autodeploy():
    ...
    try:
        ...
        # push non fatale: se sotto launchd la keychain non è accessibile, deployo comunque il commit locale
        if subprocess.run([GIT, "-C", REPO, "push", "origin", "main"]).returncode != 0:
            log("!! push Codeberg fallito (keychain?) — proseguo col deploy del commit locale")
