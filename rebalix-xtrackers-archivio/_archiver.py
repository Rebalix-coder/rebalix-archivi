...
def autodeploy():
    ...
    try:
        ...
        # ANTI-RETROCESSIONE (lezione 13 ago 2026, primo giro dalla VPS): con più macchine
        # che pushano, deployare una base non allineata a origin/main retrocede in prod il
        # lavoro delle altre. Quindi: rebase su origin/main, push, e se UNA delle due
        # fallisce il deploy è ANNULLATO (False → errore nel battito del guardiano).
        ...
        try:
            ...
            # 8 set 2026 (cautela auditor: «due modi di pubblicare = cancello aggirabile»,
            # ok Linus): il deploy passa da scripts/deploy-prod.sh --no-push — prova negativa,
...
def send_heartbeat(name, errori, modules, data_date):
    ...
    try:
        ...
        payload = json.dumps({
            ...
            "metrics": {"host": os.uname().nodename, "modules": modules, "data_date": data_date},
        }).encode("utf-8")
